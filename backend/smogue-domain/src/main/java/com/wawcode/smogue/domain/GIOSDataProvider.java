...
import com.wawcode.smogue.domain.enums.DefaultName;
import com.wawcode.smogue.domain.exception.HttpCommunicationException;
import com.wawcode.smogue.domain.exception.SmogueException;
import com.wawcode.smogue.domain.persistence.entities.*;
...
@Controller
public class GIOSDataProvider extends HttpGIOSDataReader {

    private static final Logger log = LoggerFactory.getLogger(GIOSDataProvider.class);
    ...
    private void getMeasurement(Map<Integer, StationCurrent> stations, long currentTimestamp) throws HttpCommunicationException, IOException {
        stations.entrySet().stream().parallel().forEach(entry -> {
            ...
            try {
                String response = getData(API_URL + "/get_data_chart?days="+DAYS.toString()+"&stationId="+id.toString());
    ...
    private Map<Integer, StationCurrent> getStations() throws HttpCommunicationException, IOException {
        ...
        String response = getData(API_URL + "/getAQIDetailsList?param=AQI");
    ...
    private void createMissingStations(Map<Integer, StationCurrent> stations) throws SmogueException {
        Set<CityRegion> undefinedCities = cityRepository.findByName(DefaultName.UNDEFINED_NAME.getName());

        if (undefinedCities.isEmpty()) {
            throw new SmogueException("Missing undefined city!");
        }
